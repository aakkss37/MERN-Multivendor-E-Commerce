...
const ButtonUpload: React.FC<PropType> = (props) => {
	return (
		<>
			{
				props.isLoading ?
				<label className="flex items-center justify-center my-2 px-4 py-[6px] border border-primary-dark rounded-md text-lg">
					{/* SPINNER */}
					<div className="animate-spin inline-block w-5 h-5 border-[3px] 
					border-current border-t-transparent text-primary
					rounded-full" role="status" aria-label="loading"></div>
				</label>
				:
				<label className="flex items-center justify-center my-2 px-4 py-[6px] border border-primary-dark rounded-md text-sm text-secondary-light hover:bg-white">
